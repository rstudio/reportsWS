...
1. Install
    ...
    + (for Mac users only) [Command Line Tools for XCode](http://osxdaily.com/2014/02/12/install-command-line-tools-mac-os-x/) for building packages
...
```{r}
myShinyHist <- function(data = 1:100) {
  serv <- function(input, output) {
    output$hist <- renderPlot({
      hist(data, breaks = input$n_breaks,
        probability = TRUE, xlab = "Duration (minutes)", 
        main = "Geyser eruption duration")
    })
  }
